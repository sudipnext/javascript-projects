...
const validateCred = array =>{
    ...
    let iterated = 0
    let as_it_is = 0
    for(let i = 0; i < reversed_array.length; i+=1){
        even = reversed_array[i]
        if (i % 2 === 0){
            iterated = reversed_array[i] * 2
            if (iterated > 9){
                iterated -= 9
                final_array.push(iterated)
                
            }else {
                as_it_is = iterated
                final_array.push(as_it_is)
            }
        }else {
            even_array.push(even)
        }
    }
    let new_array = [].concat(final_array, even_array, last_digit)
    sum = 0
    for (let item in new_array){
    sum += new_array[item]
